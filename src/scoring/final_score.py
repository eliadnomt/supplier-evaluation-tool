...
from .certifications import certification_bonus
from ..utils.yaml_loader import load_yaml

def final_csr_score(supplier: Supplier, config_root: str) -> Tuple[float, float]:
    caps = load_yaml(f"{config_root}/scoring.yaml").get("caps", {})
    max_score = float(caps.get("final_csr_score", 10.0))
    ...
    c_bonus = certification_bonus(supplier, config_root)

    raw = (eco * t_weight) + c_bonus
